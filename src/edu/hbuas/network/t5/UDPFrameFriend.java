...
public class UDPFrameFriend extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					UDPFrameFriend frame = new UDPFrameFriend();
	 ...
	 * Create the frame.
	 */
	public UDPFrameFriend() {
		try {
			s = new DatagramSocket(10000);
		}catch (SocketException e2) {
			e2.printStackTrace();
		}
		setTitle("UDP聊天(接收端口10085)");
		setResizable(false);
		...
		//用UDP接收信息
		try {
			DatagramSocket recive = new DatagramSocket(10085);
